...
第一版开盘区间突破过于基准化，新增 `london_gold/intraday_strategies_v2.py` 三套策略：

- `session_breakout`：伦敦/纽约真实交易时段开盘突破，带 ATR 波动率过滤
- `momentum_trend`：快/慢 ROC 双动量 + 均线趋势过滤，方向不一致时空仓
- `zscore_reversion`：波动率标准化均值回归，只沿趋势方向逆势入场

```bash
# 全参数网格 + 三策略 2/3 投票
py311 scripts/london_gold_intraday_v2_backtest.py
```

回测结论：双动量是相对最扎实的一条线，`MOM(12,24,MA48,S3)` 两年总收益 +74.19%、最大回撤 -19.10%、1051 笔、盈亏比 1.20；拆成两个样本外年度分别 +29.46% 和 +48.51%，回撤都在 -16% 左右，没有出现“训练年赚钱、测试年亏钱”的典型过拟合。伦敦/纽约时段突破和 Z-score 单独表现偏弱，三策略投票合成反而拖累收益，因此实盘建议以双动量为主、其他做过滤。20 倍杠杆满仓依然会爆仓，1%-2% 单笔风险是存活区间。
