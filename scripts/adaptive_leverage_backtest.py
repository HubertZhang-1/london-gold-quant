# -*- coding: utf-8 -*-
"""Adaptive-leverage daily bull strategy using run_leverage_backtest(leverage_series).

Compares adaptive leverage vs fixed 3x / fixed 10x per year.
"""
...
def run(part, lev, thr=0.5, stop=2.0, rr=2.0, mc=0.30):
    ...
    cost = CostConfig(capital=100_000, position_oz=10, spread=0.35, slippage=0.10,
                      ...
                      margin_call_pct=mc)
    series = None
    if isinstance(lev, str) and lev == "auto":
        series = np.clip(part["lev"].to_numpy(), 0, None)
    else:
        cost.leverage = float(lev)
    return run_leverage_backtest(f, cost, "x", leverage_series=series)["stats"]


print("=== ADAPTIVE vs FIXED 3x vs FIXED 10x (daily) ===")
print(f"{'year':>5} | {'adapt ret':>9} {'aDD':>5} | {'fx3 ret':>8} {'fx3DD':>6} | {'fx10 ret':>9} {'fx10DD':>6}")
print("-" * 66)
for wname, s, e in WINDOWS:
    ...
    f3 = run(part, 3.0)
    f10 = run(part, 10.0)
    print(f"{wname:>5} | {ad['total_return']:+9.1f} {ad['max_drawdown']:5.1f} | "
          f"{f3['total_return']:+8.1f} {f3['max_drawdown']:6.1f} | {f10['total_return']:+9.1f} {f10['max_drawdown']:6.1f}")
