# -*- coding: utf-8 -*-
"""Find a 10x-leverage bull-only config that does NOT blow up.

Root cause of -100%: too many signals (1800+/yr) with tight ATR stops under 10x
leverage -> repeated small losses -> margin_call. Fix by:
  - much higher entry threshold (fewer, higher-conviction trades)
  - wider stop (1.5-2.5 x ATR) so stops are not repeatedly hit
  - lower per-trade risk (0.3-0.5%) to buffer under leverage
  - bull + edge double gate
"""
import sys
...
def run(part, thr, stop_mult, rr, bull_thr, edge, lev, risk, mc):
    ...
    edgev = micro_s.rolling(edge, min_periods=int(edge * 0.5)).mean().fillna(0.0).to_numpy()
    sig = np.where(bull_ok & (edgev > 0.02), sig, 0)
...
print("=== 10x BULL-ONLY: fewer signals, wider stop, lower risk ===")
print(f"{'thr':>4} {'stp':>4} {'rr':>3} {'risk':>4} | {'2019':>6} {'2020':>6} {'2024':>6} {'2025':>6} {'2026':>6} | {'win%':>5} {'trades':>6}")
print("-" * 78)
    ...
    for stop in (1.5, 2.5):
        for risk in (0.003, 0.005):
            ...
            for wname, s, e in WINDOWS:
                ...
                st = run(part, thr, stop, 2.0, 0.55, 96, 10.0, risk, 0.20)
                row[wname] = st
            def f(w):
                return f"{row[w]['total_return']:+6.1f}" if w in row else "  n/a"
            tot_trades = sum(row[w]["trade_count"] for w in row)
            avg_win = np.mean([row[w]["win_rate"] for w in row if row[w]["trade_count"] > 0])
            print(f"{thr:4.2f} {stop:4.1f} {2.0:3.1f} {risk:4.3f} | {f('2019')} {f('2020')} {f('2024')} {f('2025')} {f('2026')} | {avg_win:5.0f} {tot_trades:6d}")
